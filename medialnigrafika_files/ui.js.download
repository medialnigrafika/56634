...
function rcube_splitter(p)
{
  ...
  this.init = function()
  {
	...
	if (this.handle[0].addEventListener && !!('ontouchstart' in window)) {
		// touch events fix
		this.handle[0].addEventListener('touchstart', function(e) {
			e.preventDefault();
  ...
  function onDragStart(e)
  {
	...
	if (document.addEventListener && !!('ontouchstart' in window)) {
		// touch events fix
